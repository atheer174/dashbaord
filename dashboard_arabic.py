    ...
    if emp_file and dep_file:
        try:
            ...
            with st.expander("إحصائيات إضافية"):
                ...
                # Create a pie chart for contract status distribution
                fig1, ax1 = plt.subplots()
                ax1.pie(contract_counts, labels=contract_counts.index, autopct='%1.1f%%', startangle=90)
                ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
                st.pyplot(fig1)
                st.subheader("توزيع الجنسيات")
                nat_counts = emp_df['Nationality'].value_counts()
                # Create a pie chart for nationality distribution
                fig2, ax2 = plt.subplots()
                ax2.pie(nat_counts, labels=nat_counts.index, autopct='%1.1f%%', startangle=90)
                ax2.axis('equal')
                st.pyplot(fig2)
